import { Metadata } from 'next';
import LocalPage from '@/app/_templates/LocalPage';

export const metadata: Metadata = {
  title: `Déménagement à Beaulieu (Nantes) | Devis gratuit & IA`,
  description: `Déménagement à Beaulieu (Nantes). Quartier résidentiel moderne de Nantes, proche des zones d'activité. Accès facilité. Accès: Avenues modernes, parkings nombreux, immeubles récents. Devis gratuit avec estimation IA.`,
  keywords: `déménagement Beaulieu Nantes, déménageurs Nantes, devis déménagement Nantes, Avenues modernes, parkings nombreux, immeubles récents`,
};

const nantesBeaulieuData = {
  zone: "beaulieu",
  zoneDisplay: "Beaulieu",
  description: `Quartier résidentiel moderne de Nantes, proche des zones d'activité. Accès facilité.`,
  coverImage: "/images/quartiers/nantes/beaulieu.jpg",
  accessInfo: "Avenues modernes, parkings nombreux, immeubles récents",
  pricing: {
    studio: { min: 800, max: 1200 },
    t2: { min: 1200, max: 1800 },
    t3: { min: 1800, max: 2500 },
    t4: { min: 2500, max: 3500 },
    t5: { min: 3500, max: 4500 }
  },
  destinations: [
    { name: "Nantes intra-muros", slug: "nantes" },
    { name: "Paris", slug: "paris" },
    { name: "Lyon", slug: "lyon" },
    { name: "Toulouse", slug: "toulouse" },
    { name: "France entière", slug: "france" }
  ],
  partners: [
    {
      name: "Déménageur Nantes",
      rating: 4.8,
      reviews: 69,
      specialties: ["Déménagements locaux", "Gestion des contraintes d'accès"]
    },
    {
      name: "Alex Déménagement",
      rating: 4.7,
      reviews: 45,
      specialties: ["Déménagements express", "Emballage professionnel"]
    },
    {
      name: "SAM'DÉMÉNAGE",
      rating: 4.6,
      reviews: 28,
      specialties: ["Déménagements économiques", "Service personnalisé"]
    }
  ]
};

export default function NantesBeaulieuPage() {
  return <LocalPage data={nantesBeaulieuData} />;
}
